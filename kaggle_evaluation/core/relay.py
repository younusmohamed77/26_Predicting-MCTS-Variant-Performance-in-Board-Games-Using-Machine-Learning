...
def _serialize(data) -> kaggle_evaluation_proto.Payload:
    ...
    if isinstance(data, pd.DataFrame):
        ...
        buffer = io.BytesIO()
        # write_parquet options optimized for sending many small chunks over a fast network
        data.write_parquet(buffer, compression='lz4', statistics=False)
...
def _deserialize(payload: kaggle_evaluation_proto.Payload):
    ...
    # Allowlisted special types
        ...
        return pl.read_parquet(io.BytesIO(payload.polars_dataframe_value))
    elif payload.WhichOneof('value') == 'pandas_series_value':
        # Pandas will still read a single column csv as a DataFrame.
        # ['0'] accesses the default Dataframe column name.
        return pd.Series(pd.read_parquet(io.BytesIO(payload.pandas_series_value))['0'])
